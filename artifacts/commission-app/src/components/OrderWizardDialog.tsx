...
export function OrderWizardDialog({ open, onOpenChange }: { open: boolean, onOpenChange: (o: boolean) => void }) {
  ...
  const normalizeTaxCard = (value: string) => value.replace(/\D+/g, "");
  ...
  const onTaxSubmit = async (values: z.infer<typeof taxLookupSchema>) => {
    setFoundClient(null);
    setTaxCardSearch(normalizeTaxCard(values.taxCardNumber));
  };

  // React to lookup results in an effect so render stays pure.
  useEffect(() => {
    if (!taxCardSearch || isLookupLoading || !lookupData) return;

    if (lookupData.found && lookupData.hasPendingOrder) {
      // Stay on step 1 — UI shows the blocked alert.
      setFoundClient(null);
      return;
    }

    if (lookupData.found && !lookupData.hasPendingOrder && step === 1) {
    ...
    if (!lookupData.found && step === 1) {
      setFoundClient(null);
      clientForm.setValue("taxCardNumber", taxCardSearch);
      setStep(2);
      setTaxCardSearch("");
    }
  }, [lookupData, isLookupLoading, taxCardSearch, step, clientForm]);
