...
import { ShoppingCart, DollarSign, PackageCheck, UserSquare2 } from "lucide-react";
...
export default function SalesDashboard() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{[1,2,3,4].map(i => <Skeleton key={i} className="h-32 w-full" />)}</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">{t.dashboard.salesTitle}</h2>
        <p className="text-muted-foreground mt-1">{t.dashboard.salesSubtitle}</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t.dashboard.mySalesRevenue}</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{formatCurrency(summary.totalSales, locale)}</div>
            <p className="text-xs text-muted-foreground mt-1">{t.dashboard.fromYourOrders}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{t.dashboard.earnedFromDirectSales}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t.dashboard.myClients}</CardTitle>
            <UserSquare2 className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{summary.clientCount || 0}</div>
            <p className="text-xs text-muted-foreground mt-1">{t.dashboard.assignedToYou}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div><CardTitle>{t.dashboard.recentOrders}</CardTitle><CardDescription>{t.dashboard.ordersYouSubmitted}</CardDescription></div>
            <Link href="/sales/orders"><Button variant="outline" size="sm">{t.common.viewAll}</Button></Link>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Link href="/sales/commissions"><Button variant="outline" size="sm">{t.common.viewAll}</Button></Link>
          </CardHeader>
          <CardContent>
